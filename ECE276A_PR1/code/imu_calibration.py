...
def imu_calibration_gyro(imu_data):
    ...
    bias_wx = np.mean(w_x[:200])
    bias_wy = np.mean(w_y[:200])
    bias_wz = np.mean(w_z[:200])
...
def imu_calibration_accel(imu_data):
    ...
    bias_ax = np.mean(a_x[:200])
    bias_ay = np.mean(a_y[:200])
    bias_az = (np.mean(a_z[:200])-1/scale_factor_a)
