...
def imu_calibration_gyro(imu_data):
    # Gyro calibration
    w_x = imu_data[4]
    w_y = imu_data[5]
    w_z = imu_data[6]
    ...
    bias_wx = np.mean(w_x[:100])
    bias_wy = np.mean(w_y[:100])
    bias_wz = np.mean(w_z[:100])
...
def imu_calibration_accel(imu_data):
    a_x = imu_data[1]
    a_y = imu_data[2]
    a_z = imu_data[3]
    ...
    bias_ax = np.mean(a_x[:100])
    bias_ay = np.mean(a_y[:100])
    bias_az = np.mean(a_z[:100])-1/scale_factor_a
...
def imu_calibration(imu_data):
    a_x, a_y, a_z = imu_calibration_accel(imu_data)
    w_x, w_y, w_z = imu_calibration_gyro(imu_data)
